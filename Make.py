...
args = ["csc", "-unsafe",  "-nostdlib", "-target:library", "-langversion:?"]
...
sepChar, altsepChar = SeparatorChar()

def replaceIfNecessary(origin: str) -> str:
    if(sepChar != altsepChar):
        origin.replace(altsepChar, sepChar)
    return origin


def unzip_BySys(path: str, dest: str, quiet: str):
    if os.name == "nt":
        os.system(f"7z x {path} -o{dest} -aos")
    else:
...
def parseArgs(argv):
    ...
    argParser.add_argument("--output", "-o", type=str, default="v1.4/Assemblies/RimArchive.dll", help="Output filename")
    argParser.add_argument("--csproj", type=str, default="v1.4/Source/RimArchive/RimArchive/RimArchive.csproj", help="C# project to build.")
...
def makePublicizer(p, verbose):
    ...
    if os.path.exists(ap_path):
        ...
        unzip_BySys(f"{tdir}/downloads/mono.cecil.zip",'.',quiet)
        unzip_BySys(f"{tdir}/downloads/mono.options.zip",'.',quiet)
...
def parse_packages(csproj):
    ...
    for idx, package in enumerate(csproj.getElementsByTagName("PackageReference")):
        ...
        if(version == "" or version == None):
            version = package.getElementsByTagName('Version')[0].firstChild.nodeValue
        if(version == "" or version == None):
            raise Exception(f'Cannot find \"Version\" for {name} in both attribute and childnodes')
...
def parse_sources(csproj, base_dir, verbose):
    ...
    for d, subds, files in os.walk(base_dir):
        for f in files:
            if f.endswith('.cs'):
                p = os.path.join(d,f)
                p = p.split(base_dir + sepChar, 1)[1]
                sources.append(p)
...
def parse_csproj(csproj_path, verbose):
    ...
    with XMLOpen(csproj_path) as csproj:
        ...
        source = parse_sources(csproj, base_dir, verbose)
    ...
    return packages, libraries, removed_libraries, publicized_libraries, source
...
def main(argv=sys.argv):
    ...
    os.makedirs(f"{options.reference}", exist_ok = True)
